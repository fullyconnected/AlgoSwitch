...
{
    $xqs_12="SELECT DISTINCT name,sta_id FROM geo_states WHERE con_id='".intval($_GET["country_id"])."' ORDER BY name ASC" or die(mysql_error());  
...
if($_GET["action"]=="refresh_city")
{
    ...
    $xqs_12="SELECT cty_id,name FROM geo_cities WHERE sta_id='".intval($_GET["state_id"])."' ORDER BY name ASC" or die(mysql_error());  
    $xqr_12=mysql_query($xqs_12) or die(mysql_error());
