...
class Pagination 
{
	...
	function create_links()
	{
		...
		if ($loopStart != 1){
			$output .= sprintf('<li class=""><a href="' . $link . '">&#171;</a></li>', '1'); //disabledpage
		}
		
		if ($currentPage > 1){
			$output .= sprintf('<li class=""><a href="' . $link . '">Prev</a></li>', $currentPage - 1); //nextpage
		}
		
		for ($i = $loopStart; $i <= $loopEnd; $i++)
		{
			if ($i == $currentPage){
				$output .= '<li class="">' . $i . '</li> ';//currentpage
			} else {
				$output .= sprintf('<li class=""><a href="' . $link . '">', $i) . $i . '</a></li> ';
			}
		}

		if ($currentPage < $totalPages){
			$output .= sprintf('<li class=""><a href="' . $link . '">Next</a></li>', $currentPage + 1);
		}
		
		if ($loopEnd != $totalPages){
			$output .= sprintf('<li class=""><a href="' . $link . '">&#187;</a></li>', $totalPages); //nextpage
		}

		return '<div class="pager"><ul>' . $output . '</ul></div>';
	}
}
