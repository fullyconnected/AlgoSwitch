...
// changes:
//
//  2003.02.18 - Dan Caragea:
//		- changed the <include function="function_name"> to
//		  <++include function="function_name"++>. For the visual html builders
//		  to display the text.
//
//	2003.02.11 - Dan Caragea:
//		- bug with 'remove_nonjs'. Added check for '\n' and ';' chars
//		- added the <include function="function_name"> feature. No parameters for the
//		  function supported yet.
//
//	2002.03.23
//		- bug with templates containing { a { b }
//		* v1.7.1
//
//	2002.03.03
...
class phemplate
{
	...
	*/
	...
	/*!
		search for <include filename="filename"> tags in $handle
	...
	*/
	...
	/*!
		search for <++include function="function_name"++> tags in $handle
	*/
	function include_functions($handle)	{
		$str = $this->get_var($handle);
		while(is_long($pos = strpos($str, '<++include function="'))) {
			$pos += 21;
			$endpos = strpos($str, '"++>', $pos);
			$function_name = substr($str, $pos, $endpos-$pos);
			$tag = '<++include function="'.$function_name.'"++>';
			if (strpos($function_name,'(')===false) {
				$function_name.='()';
			}
			eval("\$replacement=$function_name;");
...
*/
		...
		for ($i = 1; isset($out[$i]); $i++)
		{
			...
			else
			{
				switch ($this->unknowns)
				{
					...
					case "remove_nonjs":
							if ($out[$i]['key'] && ((false === strpos($out[$i]['key'], ' ') && (false === strpos($out[$i]['key'], "\n") && (false === strpos($out[$i]['key'], ";"))))))
							{
								$out[$i]['key'] = '';
							}
							else
	...
	*/
	function process($target, $handle, $loop = false, $include = false, $append = false, $finish = false) {
		...
		if ($include) {
			$this->set_var($target, $this->include_files($target));
		}
		...
		if ($include) {
			$this->set_var($target, $this->include_functions($target));
		}
