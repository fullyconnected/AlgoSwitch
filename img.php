...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.mb.txt');		// What to put at the end of all files in the cache directory so we can identify them
...
class timthumb {
	...
	public function __construct(){
		...
		$cachePrefix = ($this->isURL ? 'thumb_ext_' : 'thumb_int_');
		if($this->isURL){
	...
	protected function cleanCache(){
		...
		$lastCleanFile = $this->cacheDirectory . '/thumb_cacheLastCleanTime.touch';
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$imgType = "";
		$tempfile = tempnam($this->cacheDirectory, 'thumb_tmpimg_');
		...
		if($imgType == 'png' && OPTIPNG_ENABLED && OPTIPNG_PATH && @is_file(OPTIPNG_PATH)){
			...
			$exec = PNGCRUSH_PATH;
			$tempfile2 = tempnam($this->cacheDirectory, 'thumb_tmpimg_');
		...
		$tempfile4 = tempnam($this->cacheDirectory, 'thumb_tmpimg_');
	...
	protected function serveWebshot(){
		...
		$tempfile = tempnam($this->cacheDirectory, 'thumb_webshot');
		$url = $this->src;
